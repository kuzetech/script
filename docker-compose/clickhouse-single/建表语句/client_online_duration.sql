...
CREATE TABLE client_online_duration_local ON CLUSTER test_shard_localhost
(
...
ENGINE = ReplicatedReplacingMergeTree('/clickhouse/tables/{shard}/client_online_duration_local', '{replica}')
PARTITION BY toDate(time)
ORDER BY (time, log_id);

CREATE TABLE client_online_duration_all ON CLUSTER test_shard_localhost as client_online_duration_local
ENGINE = Distributed(test_shard_localhost, default, client_online_duration_local, rand());